...
# Introduction:
A data cleaning task was introduced using the hashtag #datacleaningchallenge to allow beginners, intermediate and pro data analysts in the tech space to exercise their skills using any tool of their choice.
...
I'd like you to walk with me as I take you through the step-by-step process of cleaning the dataset from messy to clean. This cleaned data can be used for analysis.

# ID, Name & Long Name: 
The ID column contains the unique number assigned to every player. The data type was converted to numbers. The ‘Name’ column contains the player's short name, while the ‘Long Name’ column contains the player's full name. The column name was changed to ‘Full Name’.
...
The ‘PhotoUrl' column and ‘PlayerUrl’ column contain the players’ photos and full names embedded in their URLs, while the ‘Nationality’ column includes the players' countries of origin. The decision to retain the ‘PhotoUrl & PlayerUrl’ column was based on the purpose of this challenge, which is to clean the data. 

![image](https://user-images.githubusercontent.com/105908253/226147150-7df4be7b-f4e5-4a95-bcca-f2c8a6652b84.png)

# Age, OVA & POT:
The ‘Age’ column contains the ages of individual players. The column was converted to a number data type. The Overall Players’ Analysis (OVA) and Players Potential (POT) columns were converted to percentages as written in the data dictionary. A custom column was created, and the value was divided by 100 to give the answer in percentage.
...
# Club:
The column consists of inconsistent data entry. Some of the club’s names had numerical prefixes attached to their names, like 1.FC Koln. The prefix was removed from the data.
...
The delimiter was changed from ‘ ~ ’ to ‘ - ‘. A conditional formatting was used to separate the players who have active contracts (2004-2021) from players on ‘loan’ and players who are ‘free’. I further created a custom column to calculate each player's contract duration. The formula below was used.
...
# Position:
The ‘Position’ column consists of one or more positions the player has ever played. The ‘position’ column was removed because another column named ‘Best Position’, which describes the best position for the player, exists in the data. The Best Position was used in Place of the Position column.

![image](https://user-images.githubusercontent.com/105908253/226147384-b139f8ea-75de-4bab-ba8c-1e5a59673a6b.png)

# Joined:
The joined column was filtered to see if there were any inconsistencies values in the data. The data were consistent and had the correct data type (date).
...
# Height & Weight:
The column contains inconsistent data. The height value was entered in two different formats. For example, 120cm & 5’10” (foot & inches), while the ‘weight’ column had values entered in two different formats- ‘kg’ and ‘lbs’.
...
The data type was converted to a whole number, and the column remained.
...
# W/F, SM & IR:
The following columns contain a rating on a scale of 1-5 with a special character ‘★’ in front of each value. The columns had the wrong data type.

![image](https://user-images.githubusercontent.com/105908253/226147515-e5024c81-4236-4bfe-8298-1451c8a25331.png)

The special character was removed, and the column names were written in full with the data type changed to a whole number.
...
# Conclusion:
After thoroughly checking and cross-checking for errors, missing values, empty cells, outliers, and wrong data type. I can say the messy dataset is now clean and consistent, which can now be used for analysis.
I’m glad I participated in this challenge to hone and sharpen my skills. Quite a lot of learning I did while taking up this task. What initially looked difficult and impossible has been done. It's good to say I’m proud of myself.


Thank you for reading through. I hope you find this worth reading and helpful. If you need my support in working on this dataset, you can reach me on Twitter [@pinnacle_paul](https://twitter.com/pinnacle_paul) and LinkedIn [@PaulAderounmu](https://www.linkedin.com/in/pauladerounmu)
